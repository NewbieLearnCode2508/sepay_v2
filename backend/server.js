...
db.serialize(() => {
    ...
    // Bảng deposit_transactions (giữ nguyên)
    ...
    // Bảng lịch sử mua (thay thế purchase_orders cũ)
    ...
    db.get(`SELECT COUNT(*) as count FROM game_accounts`, (err, row) => {
        if (err) {
            console.error('Lỗi kiểm tra game_accounts:', err.message);
            return;
        }
        if (row.count === 0) {
            const stmt = db.prepare(`INSERT INTO game_accounts (game_name, username, password, price) VALUES (?, ?, ?, ?)`);
            // Liên Quân Mobile: 10 acc
            for (let i = 1; i <= 10; i++) {
                stmt.run('Liên Quân Mobile', `lq_user${i}`, `pass${i}`, 50000);
            }
            // Free Fire: 10 acc
            for (let i = 1; i <= 10; i++) {
                stmt.run('Free Fire', `ff_user${i}`, `ffpass${i}`, 30000);
            }
            // Genshin Impact: 10 acc
            for (let i = 1; i <= 10; i++) {
                stmt.run('Genshin Impact', `gi_user${i}`, `gipass${i}`, 80000);
            }
            stmt.finalize();
...
// ========== API GAME ==========
// Lấy danh sách game (group theo tên, số lượng còn, giá)
app.get('/api/games', (req, res) => {
    db.all(`
    SELECT game_name, price, COUNT(*) as available
    FROM game_accounts
    WHERE status = 'available'
    GROUP BY game_name, price
  `, (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
...
app.post('/api/purchase', authenticateToken, (req, res) => {
    ...
    // Lấy một tài khoản available random của game đó
    db.get(`
    SELECT id, price, username FROM game_accounts
    WHERE game_name = ? AND status = 'available'
    ORDER BY RANDOM() LIMIT 1
  `, [game_name], (err, account) => {
        if (err || !account) return res.status(404).json({ error: 'Không còn tài khoản nào cho game này' });
        ...
        db.get('SELECT balance FROM users WHERE id = ?', [req.user.id], (err, user) => {
            ...
            // Trừ tiền và đánh dấu account đã bán
            db.run('UPDATE users SET balance = balance - ? WHERE id = ?', [total, req.user.id], (err) => {
                ...
                db.run('UPDATE game_accounts SET status = "sold" WHERE id = ?', [account.id], (err) => {
                    ...
                    // Lưu lịch sử mua
                    db.run(
                        'INSERT INTO purchase_orders (user_id, account_id, total_amount) VALUES (?, ?, ?)',
                        [req.user.id, account.id, total],
                        (err) => {
                            if (err) return res.status(500).json({ error: err.message });
...
// ========== LỊCH SỬ MUA & XEM MẬT KHẨU ==========
app.get('/api/purchase-history', authenticateToken, (req, res) => {
    db.all(`
    SELECT po.id, po.total_amount, po.created_at,
           ga.game_name, ga.username, ga.password
    FROM purchase_orders po
    JOIN game_accounts ga ON po.account_id = ga.id
    WHERE po.user_id = ?
    ORDER BY po.created_at DESC
  `, [req.user.id], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        // Không gửi password về ngay lập tức, chỉ gửi khi yêu cầu riêng
        const safeRows = rows.map(row => ({
            ...
            has_password: !!row.password  // ẩn password, chỉ cho biết có tồn tại
        }));
        res.json(safeRows);
    });
});

// API lấy mật khẩu (chỉ khi user sở hữu tài khoản đó)
app.get('/api/account-password/:purchase_id', authenticateToken, (req, res) => {
  ...
  `, [purchase_id, req.user.id], (err, row) => {
...
app.post('/api/deposit', authenticateToken, (req, res) => {
    ...
    db.run(`INSERT INTO deposit_transactions (user_id, order_code, amount) VALUES (?, ?, ?)`,
        [req.user.id, order_code, amount],
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/deposit-status/:order_code', authenticateToken, (req, res) => {
    ...
    db.get(`SELECT status FROM deposit_transactions WHERE order_code = ? AND user_id = ?`,
        [order_code, req.user.id], (err, row) => {
            if (err || !row) return res.status(404).json({ error: 'Không tìm thấy' });
            res.json({ status: row.status });
        });
});

// Webhook SePay (giữ nguyên)
app.post('/webhook/sepay', (req, res) => {
    ...
    db.get(
        `SELECT id, user_id FROM deposit_transactions WHERE order_code = ? AND amount = ? AND status = 'pending'`,
        [order_code, amount],
        (err, transaction) => {
